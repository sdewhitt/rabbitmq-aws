...
%%--------------------------------------------------------------------
%% client_credentials token-fetch tier: pure-phase validation
%%--------------------------------------------------------------------

%% token_endpoint present but client_id + client_secret_arn absent -> input_invalid.
grant_missing_client_fields_test() ->
    Body = #{
        <<"jwks_uri">> => <<"https://idp.example.com/jwks">>,
...
%% token_endpoint + client_id present but client_secret_arn absent -> input_invalid.
grant_missing_secret_arn_test() ->
    Body = #{
        <<"jwks_uri">> => <<"https://idp.example.com/jwks">>,
        <<"token_endpoint">> => <<"https://idp.example.com/oauth2/token">>,
...
%% A plain http:// token_endpoint is rejected (https-only, same as jwks_uri).
grant_http_token_endpoint_rejected_test() ->
    Body = #{
        <<"jwks_uri">> => <<"https://idp.example.com/jwks">>,
...
%% A token_endpoint whose literal host is IMDS is rejected by the SSRF guard in
%% the pure phase, before any ARN resolve or outbound request.
grant_token_endpoint_ssrf_denied_test() ->
    Body = #{
        <<"jwks_uri">> => <<"https://idp.example.com/jwks">>,
        <<"token_endpoint">> => <<"https://169.254.169.254/oauth2/token">>,
        <<"client_id">> => <<"client-abc">>,
        <<"client_secret_arn">> => <<"arn:aws:secretsmanager:us-west-2:1:secret:s">>
    },
    ?assertMatch({error, input_invalid, _}, aws_auth_validate_oauth:validate(Body)).

%% scopes as a non-empty list of binaries normalizes to a space-delimited binary.
parse_scopes_list_test() ->
    ?assertEqual(
        {ok, <<"a b c">>},
        aws_auth_validate_oauth:parse_scopes([<<"a">>, <<"b">>, <<"c">>])
    ).

%% scopes as a list containing a non-binary is rejected.
parse_scopes_bad_list_test() ->
    ?assertMatch(
        {error, input_invalid, _},
        aws_auth_validate_oauth:parse_scopes([<<"a">>, 5])
    ).

%% absent scopes -> none (the grant omits the scope field).
parse_scopes_absent_test() ->
    ?assertEqual({ok, none}, aws_auth_validate_oauth:parse_scopes(undefined)).

%% The form body percent-encodes every value so a secret cannot break the field.
build_grant_body_encodes_test() ->
    Body = aws_auth_validate_oauth:build_grant_body(
        <<"client-abc">>, <<"s cr&et=/">>, none
    ),
    %% grant_type + client_id + client_secret present; the secret's reserved
    %% characters are percent-encoded (no raw '&' or '=' from the secret value).
    ?assert(is_binary(Body)),
    ?assertNotEqual(nomatch, string:find(binary_to_list(Body), "grant_type=client_credentials")),
    ?assertNotEqual(nomatch, string:find(binary_to_list(Body), "client_secret=")),
    %% The secret's '&' must be encoded (%26), not left literal.
    ?assertNotEqual(nomatch, string:find(binary_to_list(Body), "%26")).

%% has_access_token: a 200 body with a non-empty access_token -> true.
has_access_token_true_test() ->
    Body = rabbit_json:encode(#{<<"access_token">> => <<"tok">>, <<"token_type">> => <<"Bearer">>}),
    ?assert(aws_auth_validate_oauth:has_access_token(Body)).

%% has_access_token: a 200 body without access_token -> false.
has_access_token_false_test() ->
    Body = rabbit_json:encode(#{<<"error">> => <<"invalid_client">>}),
    ?assertNot(aws_auth_validate_oauth:has_access_token(Body)).

%%--------------------------------------------------------------------
%% client_credentials token-fetch tier: network phase (mocked)
%%--------------------------------------------------------------------

%% Full happy path: valid JWKS AND a token endpoint returning access_token -> ok.
%% The client secret is resolved via a mocked ARN under a configured assume_role.
grant_fetch_success_test_() ->
    {setup, fun setup_httpc_mock/0, fun teardown_httpc_mock/1, fun(_) ->
        setup_assume_role(),
        meck:expect(aws_arn_util, resolve_arn, fun(_Arn, State) ->
            {ok, <<"the-secret">>, State}
        ...
        TokenBody = rabbit_json:encode(#{<<"access_token">> => <<"tok-123">>}),
        mock_two_endpoints(JwksBody, 200, TokenBody),
        Result = aws_auth_validate_oauth:validate(grant_body()),
        teardown_assume_role(),
        [?_assertEqual(ok, Result)]
    end}.

%% The token endpoint rejects the grant (401) -> auth_failed.
grant_fetch_rejected_test_() ->
    {setup, fun setup_httpc_mock/0, fun teardown_httpc_mock/1, fun(_) ->
        setup_assume_role(),
        meck:expect(aws_arn_util, resolve_arn, fun(_Arn, State) ->
            {ok, <<"the-secret">>, State}
        ...
        mock_two_endpoints(JwksBody, 401, <<"{\"error\":\"invalid_client\"}">>),
        Result = aws_auth_validate_oauth:validate(grant_body()),
        teardown_assume_role(),
        [?_assertMatch({error, auth_failed, _}, Result)]
    end}.

%% A token endpoint 200 that carries no access_token -> auth_failed.
grant_fetch_no_token_test_() ->
    {setup, fun setup_httpc_mock/0, fun teardown_httpc_mock/1, fun(_) ->
        setup_assume_role(),
        meck:expect(aws_arn_util, resolve_arn, fun(_Arn, State) ->
            {ok, <<"the-secret">>, State}
        ...
        mock_two_endpoints(JwksBody, 200, <<"{\"token_type\":\"Bearer\"}">>),
        Result = aws_auth_validate_oauth:validate(grant_body()),
        teardown_assume_role(),
        [?_assertMatch({error, auth_failed, _}, Result)]
    end}.

%% Referencing client_secret_arn with NO configured assume_role is refused with
%% config_conflict BEFORE any ARN resolve or token request (guardrail applies to
%% the top-level secret ARN, not just ssl_options ARNs).
grant_secret_arn_without_assume_role_test_() ->
    {setup, fun setup_httpc_mock/0, fun teardown_httpc_mock/1, fun(_) ->
        application:unset_env(aws, arn_config),
        meck:expect(aws_arn_util, resolve_arn, fun(_Arn, _State) ->
            erlang:error(secret_resolved_without_assume_role)
        end),
        Result = aws_auth_validate_oauth:validate(grant_body()),
        [
            ?_assertMatch({error, config_conflict, _}, Result),
...
%% R6: neither the resolved client secret nor the fetched access token may
%% appear in the rendered result on the success path.
grant_no_secret_or_token_leak_test_() ->
    {setup, fun setup_httpc_mock/0, fun teardown_httpc_mock/1, fun(_) ->
        setup_assume_role(),
        meck:expect(aws_arn_util, resolve_arn, fun(_Arn, State) ->
            {ok, <<"SUPER-SECRET-VALUE">>, State}
        end),
        JwksBody = rabbit_json:encode(#{
            <<"keys">> => [#{<<"kty">> => <<"RSA">>, <<"kid">> => <<"k1">>}]
        }),
        TokenBody = rabbit_json:encode(#{<<"access_token">> => <<"LEAKY-TOKEN-VALUE">>}),
        mock_two_endpoints(JwksBody, 200, TokenBody),
        Result = aws_auth_validate_oauth:validate(grant_body()),
        Rendered = lists:flatten(io_lib:format("~p", [Result])),
        teardown_assume_role(),
        [
            ?_assertEqual(ok, Result),
            ?_assertEqual(nomatch, string:find(Rendered, "SUPER-SECRET-VALUE")),
            ?_assertEqual(nomatch, string:find(Rendered, "LEAKY-TOKEN-VALUE"))
        ]
    end}.
...
%% A body that activates the client_credentials tier (jwks_uri + token grant).
grant_body() ->
    #{
        <<"jwks_uri">> => <<"https://idp.example.com/.well-known/jwks.json">>,
        <<"token_endpoint">> => <<"https://idp.example.com/oauth2/token">>,
...
%% Configure an assume_role so resolving the client_secret_arn is permitted, and
%% mock aws_iam:assume_role to thread the state through unchanged.
setup_assume_role() ->
    application:set_env(aws, arn_config, [
...
%% Mock httpc so the JWKS endpoint returns JwksBody (200) and the token endpoint
%% returns {TokenStatus, TokenBody}. The two are distinguished by URL: the token
%% endpoint path contains "token".
mock_two_endpoints(JwksBody, TokenStatus, TokenBody) ->
    meck:expect(httpc, request, fun(_Method, Req, _HttpOpts, _Opts, _Profile) ->
        Url = request_url(Req),
        case string:find(Url, "oauth2/token") of
            nomatch ->
                {ok, {{"HTTP/1.1", 200, "OK"}, [], JwksBody}};
            _ ->
                {ok, {{"HTTP/1.1", TokenStatus, "OK"}, [], TokenBody}}
        end
    end).
