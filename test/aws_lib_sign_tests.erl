...
amz_date_test_() ->
    [
        {"value", fun() ->
            ?assertEqual(
                "20160220",
                aws_lib_sign:amz_date("20160220T120000Z")
            )
        end}
    ].
...
canonical_headers_test_() ->
    [
        {"with security token", fun() ->
            ...
            ?assertEqual(Expectation, aws_lib_sign:canonical_headers(Value))
        end}
    ].
...
credential_scope_test_() ->
    [
        {"string value", fun() ->
            ...
            ?assertEqual(
                Expectation,
                aws_lib_sign:credential_scope(RequestDate, Region, Service)
            )
        end}
    ].
    ...
    [
        {"signed value", fun() ->
            ?assertEqual(
                ...
                aws_lib_sign:hmac_sign("sixpence", "burn the witch")
            )
        end}
    ].
...
query_string_test_() ->
    [
        {"properly sorted", fun() ->
            ...
            ?assertEqual(
                Expectation,
                aws_lib_sign:query_string(QArgs)
            )
        end},
        {"undefined", fun() ->
            ?assertEqual([], aws_lib_sign:query_string(undefined))
        end}
    ].
...
request_hash_test_() ->
    [
        {"hash value", fun() ->
            ...
            ?assertEqual(
                Expectation,
                aws_lib_sign:request_hash(Method, Path, QArgs, Headers, Payload)
            )
        end}
    ].
...
signature_test_() ->
    [
        {"value", fun() ->
            ...
            ?assertEqual(Expectation, aws_lib_sign:signature(StringToSign, SigningKey))
        end}
    ].
...
signed_headers_test_() ->
    [
        {"with security token", fun() ->
            ...
            ?assertEqual(Expectation, aws_lib_sign:signed_headers(Value))
        end}
    ].
...
signing_key_test_() ->
    [
        {"signing key value", fun() ->
            ...
            ?assertEqual(
                Expectation,
                aws_lib_sign:signing_key(SecretKey, AMZDate, Region, Service)
            )
        end}
    ].
...
local_time_1_test_() ->
    [
        {"variation1", fun() ->
            ...
            ?assertEqual(Expectation, aws_lib_sign:local_time(Value))
        end},
        {"variation2", fun() ->
